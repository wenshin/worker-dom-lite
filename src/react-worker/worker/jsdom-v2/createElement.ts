...
export function getHTMLElementInterface(name) {
...
export function getValidTagNames(namespace, name) {
...
function _createElement(document, localName, namespace, prefix = null, isValue = null, synchronousCE = false) {
  let result = null;
  ...
  if (definition !== null && definition.name !== localName) {
    ...
    result = new Element(_globalObject, [], {
...
export function internalCreateElementNSSteps(document, namespace, qualifiedName, options) {
...
export type ProxyElement = Element & { $wid: string };

export function createElement(tag: string, doc: Bridge): ProxyElement {
  const agent = _createElement();
