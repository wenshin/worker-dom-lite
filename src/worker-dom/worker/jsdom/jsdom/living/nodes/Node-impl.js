...
class NodeImpl extends EventTargetImpl {
  ...
  _preInsert(nodeImpl, childImpl) {
    ...
    const isNoNeedChildren = [ 'html', 'head', 'body', 'script' ].indexOf(nodeImpl.nodeName.toLowerCase()) > -1;
    const isNoNeedParent = [ '#document', 'html' ].indexOf(this.nodeName.toLowerCase()) > -1;
    const isEmptyText = this.nodeName.toLowerCase() === 'body' && nodeImpl.nodeName === '#text';
    if (!isNoNeedParent && !isNoNeedChildren && !isEmptyText) {
      this.$bridge.invoke('Node:insertBefore', [
        this.$cargo,
        nodeImpl.$cargo,
        referenceChildImpl && referenceChildImpl.$cargo
      ]);
    }
